  ...
  const addTemplate = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        addAdminLog('Template Success', `Template "${newTemplateName}" berhasil disimpan permanen oleh ${username}`);
        ...
        setTimeout(() => loadTemplates(true), 1000);
      } else {
        addAdminLog('Template Error', `Gagal menambah template: ${data.error || 'Unknown error'}`);
      }
    } catch (error) {
